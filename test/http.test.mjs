...
// queried over HTTP. Global fetch is replaced by the same mock the other
// contracts use, injected with --import.
//
// The door itself is shared with the Proton server by construction (http.mjs
// is derived from it), so these cases are the guard against that derivation
// silently losing one of its four hard-won details.
...
function start(env = {}) {
  return new Promise((done, fail) => {
    const p = spawn(process.execPath, ['--import', MOCK, HTTP], {
      env: {
        ...process.env,
        HOME: tmp,
        GMAIL_MCP_CONFIG_DIR: wurzel,
        GMAIL_MCP_ATTACHMENT_DIR: path.join(tmp, 'http-attachments'),
        GMAIL_MCP_TOKEN_FILE: tokenFile,
        GMAIL_MCP_PORT: '0',
        FETCH_MOCK_LOG: MOCK_LOG,
        FETCH_MOCK_RESPONSES: JSON.stringify({
          'oauth2.googleapis.com/token': { body: { access_token: 'z', expires_in: 3600 } },
          'users/me/labels': { body: { labels: [] } },
        }),
        ...env,
      },
...
test('With the right token the MCP server answers', async () => {
  ...
  assert.ok(r.text.includes('gmail'), r.text.slice(0, 200));
});
...
test('A tool call reaches the Gmail API and nowhere else', async () => {
  const init = await post(INIT);
  const session = init.session;
  await post({ jsonrpc: '2.0', method: 'notifications/initialized' }, { session });
  ...
  await post({
    ...
    params: { name: 'labels_list', arguments: { account: 'probe' } },
  }, { session });
  const lines = fs.readFileSync(MOCK_LOG, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
  assert.ok(lines.length > 0, 'no request was made');
  for (const r of lines) {
    assert.ok(r.url.startsWith('https://gmail.googleapis.com/')
      || r.url.startsWith('https://oauth2.googleapis.com/'), r.url);
  }
});
...
test('Without a token file the server does not start at all', async () => {
  await assert.rejects(
    () => start({ GMAIL_MCP_TOKEN_FILE: path.join(tmp, 'does-not-exist') }),
    /exited with/,
  );
});
...
test('A token file readable by others is refused', async () => {
  ...
  await assert.rejects(
    () => start({ GMAIL_MCP_TOKEN_FILE: open }), /exited with/);
});
...
test('A token that is too short is refused', async () => {
  ...
  await assert.rejects(
    () => start({ GMAIL_MCP_TOKEN_FILE: short }), /exited with/);
});
