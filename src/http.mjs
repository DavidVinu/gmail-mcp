#!/usr/bin/env node
// Gmail over Streamable HTTP, for MCP clients that connect to a URL.
//
...
// rule for reply drafts. The door below is the same one the Proton server
// uses, deliberately: its four hard-won details (404 rather than 400 on an
// unknown session, logging the status the response actually had, no
// WWW-Authenticate, 404 on the OAuth discovery paths) were each paid for by a
// real outage, and re-deriving them here would only re-earn them.
//
...
const CONFIG = loeseKonfig();

const TOKEN_FILE = process.env.GMAIL_MCP_TOKEN_FILE
  ?? `${process.env.HOME}/.config/gmail-mcp/http-token`;
const PORT = Number(process.env.GMAIL_MCP_PORT ?? 18791);
const ADDRESS = process.env.GMAIL_MCP_ADDRESS ?? '127.0.0.1';
const MIN_LENGTH = 32;

function bail(reason) {
  process.stderr.write(`gmail-mcp: ${reason}\n`);
  process.exit(2);
}
...
function log(req, status, note = '') {
  ...
  process.stderr.write(
    `gmail-mcp: ${status} ${req.method} ${req.url} `
...
const service = http.createServer(async (req, res) => {
  ...
  try {
    ...
    process.stderr.write(`gmail-mcp: ${error?.message ?? error}\n`);
    if (!res.headersSent) {
...
service.listen(PORT, ADDRESS, () => {
  ...
  process.stderr.write(`gmail-mcp: ready on http://${ADDRESS}:${port}/mcp\n`);
});
