...
//
// Copied verbatim from the protonmail-mcp repo, deliberately: the two servers
// are separate deployments with separate blast radii, and a shared library
// between them would make an edit for one silently change the other. The
// differential test against the Python original lives in that repo.
//
...
// Gmail identifies messages, threads and drafts by opaque hex strings and
// history records by decimal ids. None of them can carry a one-time code,
// and destroying them makes every follow-up call impossible.
export const STRUCTURAL_FIELDS = ['id', 'uid', 'threadId', 'messageId',
  'historyId', 'attachmentId', 'draftId'];
const KENNUNG = /^[0-9a-f]+$/i;

function isUid(key, value, structuralFields) {
  return structuralFields.includes(key)
    && typeof value === 'string' && KENNUNG.test(value);
}
